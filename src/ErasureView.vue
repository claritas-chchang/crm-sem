...
const filteredRecords = computed(() => {
  ...
  return props.records.filter(r => {
    ...
    if (selectedFilter.value === 'Area') return r.area?.toLowerCase().includes(q)
    if (selectedFilter.value === 'Shift') return r.shift?.toLowerCase().includes(q)
    return false
  })
})
...
<template>
  <div class="top-record-box no-margin custom-erasure-view">
    <div class="sub-header box-header">
      ...
      <span style="font-size: 14px; font-weight: bold; color: #333;">ERASURE RECORDS</span>
    </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              >
                ...
                <td><a href="#" class="item-link" @click.prevent="openDetail(record)">{{ record.id }}</a></td>
                <td>{{ record.date }}</td>
                <td>{{ record.shift }}</td>
                <td>{{ record.type }}</td>
                <td>{{ record.area }}</td>
                ...
                <td>{{ record.createdTs }}</td>
                <td>{{ record.createdBy }}</td>
              </tr>
              <tr v-if="filteredRecords.length === 0">
                <td colspan="13" style="text-align: center; padding: 20px; color: #666;">No records found.</td>
              </tr>
            </tbody>
          </table>
        </div>
