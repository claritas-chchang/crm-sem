...
const filteredRecords = computed(() => {
  ...
  return internalRecords.value.filter(r => {
    const field = selectedFilter.value || 'formNo'
...
<template>
  <div class="top-record-box no-margin custom-equipment-view">
    <div class="sub-header box-header">
      ...
      <span style="font-size: 14px;">EQUIPMENT RECORDS</span>
    </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              >
                ...
                <td><a href="#" class="item-link" @click.prevent="openDetail(record)">{{ record.formNo }}</a></td>
                <td>{{ record.subNo }}</td>
              ...
              <tr v-if="filteredRecords.length === 0">
                <td colspan="11" style="text-align: center; padding: 20px; color: #666;">No equipment records found.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
.selected-text { font-size: 12px; color: var(--text-main); }
.search-row {
  background-color: #fff;
}
...
.page-text, .display-text { font-size: 12px; color: var(--text-main); }
